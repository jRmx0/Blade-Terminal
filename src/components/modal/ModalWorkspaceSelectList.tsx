...
export default function ModalWorkspaceSelectList({
    ...
    const deletePadding = onDelete ? "pr-10" : "";

    return (
        <ul className="border border-gray-300 bg-white h-52 overflow-y-auto rounded">
            {/* Sticky column header */}
            <li className={`sticky top-0 flex items-center px-3 py-1 bg-gray-200 text-sm font-bold text-gray-700 tracking-wider border-b border-gray-300 z-10 cursor-default ${deletePadding}`}>
                ...
                environments.map((env) => {
                    ...
                    return (
                        <li key={env.id} className="group relative">
                            <button
                                type="button"
                                className={`flex items-center w-full px-3 py-1.5 text-sm text-left cursor-pointer transition-colors ${deletePadding} ${isSelected ? "bg-teal-600 text-white" : "hover:bg-gray-100 text-gray-700"
                            ...
                            {onDelete && (
                                <button
                                    type="button"
                                    className={`absolute right-2 top-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 p-0.5 rounded transition-opacity leading-none cursor-pointer ${isSelected ? "text-teal-100 hover:text-white hover:bg-teal-700" : "text-gray-400 hover:text-red-500 hover:bg-gray-200"
                                        }`}
