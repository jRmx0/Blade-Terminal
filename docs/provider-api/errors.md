...
Providers must conform to this error shape whenever returning an error response. blade-terminal uses these codes to surface meaningful messages to the user.

---

## Error Response Shape

All HTTP-layer errors must use this structure:
...
Each algorithm validates its own request and emits unique error codes. The following is an example — actual codes depend on which algorithm ran. Check the `algorithmName` field on the failed job to identify the relevant algorithm.
