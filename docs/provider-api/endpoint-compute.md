...
blade-terminal submits compute jobs over HTTP and polls for results asynchronously. Providers must implement both endpoints exactly as described here.

---

## `POST /compute`

Submits a compute job. Must return immediately with a job ID — computation runs asynchronously.
...
Polls the status of a previously submitted job. blade-terminal polls this endpoint every ~600 ms for up to 60 seconds.
...
|---|---|---|---|
| `algorithmId` | `number` | yes | Must match `algorithm.id` from `GET /metadata`. The provider routes to the correct algorithm by this integer ID. |
...
| `realworld` | `object` | no | Optional original geometry snapshot. Present when blade-terminal sends transformed `environment` geometry (e.g., headland-adjusted polygons). `algo_bounce` uses this geometry for coverage when provided; otherwise it falls back to `environment`. |
...
Parameter constraints (allowed ranges, required enum values, etc.) are algorithm-specific. They are validated by the algorithm's native core and reported as job-level error codes when violated — see [errors.md](./errors.md).
...
Extensible string enum. Providers must emit only defined values. blade-terminal accepts unknown values without error and renders them neutrally.
...
| Field | Type | Notes |
|---|---|---|
| `id` | `number` | Matches `layer.id` from `GET /metadata` — used to look up style and display config |
